...
평평한 수면 아래의 점 물체를 수면 위에서 바라볼 때, 상의 위치는 시점에 따라 달라진다. 
...
\begin{figure}
	...
	\caption{코스틱: 굴절광선의 연장선들이 형성하는 포락선}
	\label{fig:caustic}
\end{figure}

물체와 시점을 포함하는 법평면에서, \figurename\ \ref{fig:caustic}\과 같이 법평면과 수면의 교선을 $x$-축으로, 
물체를 통과하는 법선을 $y$-축으로 하자. 그러면 상의 자취는 다음 곡선의 일부이다. \cite{Glaeser2000}
...
굴절 광선의 연장선은 $y$축과 점 B$(0, b)$에서 만난다. 입사각 $\theta_2$가 변할 때 직선 ${\mathrm{AB}}$들이 중첩하여 포락선을 형성하는데, 이렇게 광선들이 모여 만드는 포락선을 코스틱 곡선이라고 한다.\footnote{이 경우에는 실제 광선들이 아닌 광선들의 연장선이 만드는 가상의 코스틱, 즉 허코스틱이다.} 상은 직선 ${\mathrm{AB}}$와 코스틱의 접점 $\mathrm{C}$에 위치한다.  이는 이 점 근처의 광선 다발이 국지적으로 발산하는 점이기 때문이다. 
...
인 곡선족인데, \href{http://dynamicmathematicslearning.com/super-ellipse.html}%
...
{\emph{superellipse}})이라는 곡선족과 함께 묶어
`초원뿔곡선'(\emph{superconics}) 이라는 집합적인 명칭으로 부르고 있는, 광범위한 곡선족의 일부이다.
...
방정식의 형태에서 알 수 있듯이 상의 좌표는 시점의 좌표에 의존한다. 예를 들어 물속에 반쯤 잠긴 연필 끝점의 상은 그림 \ref{fig:pencil_view}\와 같이 시점 $1$, $2$에 따라 각각 $1'$, $2'$로 위치가 달라진다.
...
라 할 수 있다. 
$a$ 값이 변하면 두 점을 잇는 직선도 변한다. 이러한 직선족이 이루는 포락선은 $a$가 극소량 변할 때 변하지 않는 정류점, 즉
