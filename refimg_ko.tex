...
이는 한편으로는 이 현상이 비교적 복잡한 수학적 처리를 요구하므로 입문 과정에서 다루기에는 적절하지 않고, 다른 한편으로는 렌즈나 거울과 같은 광학 기기의 작동 원리에 비해 상대적으로 중요성이 낮게 평가되기 때문에 고급 과정에서 다루기에도 알맞지 않기 때문일 것이다. 
...
입사각 $\theta_2$가 변할 때 굴절광선 $\overline{\mathrm{AB}}$들이 중첩하여 포락선을 형성하는데, 이렇게 광선들이 모여 만드는 포락선을 코스틱이라고 한다.\footnote{이 경우에는 실제 광선들이 아닌 광선들의 연장선이 만드는 가상의 코스틱, 즉 허코스틱이다.} 상은 선분 $\overline{\mathrm{AB}}$와 코스틱의 접점 $\mathrm{C}$에 위치한다. 
...
매개변수를 $\alpha$로 하는 선분 $\overline{\mathrm{\Aprime\Bprime}}$들을 중첩시켰을 때 형성되는 포락선은 `아스트로이드'(%
...
선분 $\overline{\mathrm{\Aprime\Bprime}}$과
...
물리학적으로도 의미가 있고, 아스트로이드에 대한 이 곡선의 관계가 타원에 대한 쌍곡선과의 관계와 
...
각각 이와 같은 방법으로 상 $1', '2, 3', \dots$을 찾을 수 있다. 점이 연속으로 움직일 때 따라 움직이는 상의 자취가 
물체의 상이 될 것이다.
