...
興味はあるが、あまり時間をかけたくない読者のために結論から述べると、平らな水面下の点光源を水面上から見ると、像の位置は視点によって変化する。視点が動くにつれて法平面内で観察される像の軌跡は、一種のカウスティック曲線であり、この場合、変形したアストロイドと呼ばれる曲線となる。
...
屈折光線の延長線は$y$軸と点B$(0, b)$で交わる。入射角$\theta_2$が変化するとき、直線${\mathrm{AB}}$が重なり合い包絡線を形成し、このように光線が集まってできる包絡線をカオスティック曲線と呼ぶ。\footnote{この場合、実際の光線ではなく、光線の延長線が作る仮想的なカオスティック、すなわち虚カオスティックである。} 像は線分${\mathrm{AB}}$とカオスティックの接点$\mathrm{C}$に位置する。これは、この点の近傍の光線束が局所的に発散する点だからである。

\begin{figure}
	\centering
	\includegraphics[width=3in]{figs/g237.eps}
	\caption{空気と水の界面での光の屈折と、その延長線とカオスティックの幾何学的関係}
	\label{fig:geometry}
\end{figure}
...
\begin{figure}
	...
	\caption{$\xi\eta$ 平面への投影で得られるコーティックは、アストロイドである。}
	\label{fig:astroid}
\end{figure}
...
物体が境界面から高さDの空気中にあり、観測者が水中にある場合、相対的な屈折率は<span class="math-inline">1/n < 1</span>となり、同様の推論により、コーティックに関する以下の式を得る（付録\ref{app:hyperastroid}参照）。
$$ \left| \xi \right|^{2/3} - \left| \eta \right|^{2/3} = -1, $$
...
で表される曲線族に属し、\href{http://dynamicmathematicslearning.com/super-ellipse.html}{DML}\footnote{\ttfamily{dynamicmathematicslearning.com/super-ellipse.html}}では\emph{Super Hyperbola}と呼ばれ、\href{https://old.nationalcurvebank.org/superconicncb/superconicncb.htm}{National Curve Bank}\footnote{\ttfamily{https://old.nationalcurvebank.org/superconicncb/superconicnb.htm}}ではアストロイドが属する
$$ \left| \xi \right|^{r} + \left| \eta \right|^{r} = 1. $$
のような曲線族である\emph{superellipse}とまとめて\emph{superconics}と呼ばれる、広範な曲線族の一部である。

特に $r = 2/3$ のとき、
$$ \left| \xi \right|^{2/3} - \left| \eta \right|^{2/3} = \pm1 $$
となる特別な場合があり、これは水面の点光源から出て水中に進入し屈折する光線のコーティック曲線として物理学的に意味を持ち、アストロイドとの関係が楕円と双曲線の関係に似ていることから、ここでは\emph{hyperastroid}と呼ぶことにする。
	
\section{像の位置を求める}

物体の位置と視点が与えられたとき、カオスティックを利用して像の位置を以下のように求めることができる。

\begin{figure}[!h]
	\centering
	\includegraphics[width=2.7in]{figs/g394.eps}
	\caption{カオスティックを利用した像の位置決定}
	\label{fig:image_caustic}
\end{figure}

図\ref{fig:image_caustic}のように、視点(POV)からカオスティックに接線を引く。接線とカオスティックの接点($O'$)が像の位置であり、接線が水面と交わる点が物体($O$)から出た光線が水面に入射する点である。
...
\]
を得る。複素数解全体では4つの解を持ち、実数解は一般に2つであるが、このうち $0$ と $\xi_{\mathrm{V}}$ の間のものを選べば、それが求める $\alpha$ の値である。一般に4次方程式を解くことは解析的に行うのが難しいため、コンピュータ代数システム (CAS) を利用して解析解を求めることも可能だが、大きな利点はない。数値的な方法で近似解を求める方が効率的である場合が多い。 (付録 \ref{app:python} 参照)
...
前節で像の位置を定量的に求める方法を学んだので、この方法を用いて、水中空間を水中から見たときの像がどのように歪んで見えるかを調べてみよう。図\ref{fig:grid_underwater}のように正方形格子で表された水中の長方形領域は、水面から見下ろすとき、視点の相対的な高さによって図\ref{fig:image_underwater}〜\ref{fig:seashell}のように見える。
