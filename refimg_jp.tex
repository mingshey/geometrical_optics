...
%		{n^3\romana^3}\left\{ D^2-(n^2-1)\romana^2 \right\}^{3/2}\\
	...
	\begin{figure}[h]
		...
		\caption{水面に半分浸かった箸の写真：水面に近い低い角度から撮影}
		\label{fig:picture}
	\end{figure}
	
	別の方法として、フェルマーの原理を用いて光線と水面の交点を求め、次にアストロイドの接点の式を用いて像の位置を決定する方法があります。
	
	どちらの方法でも4次方程式の解を求める過程が必要ですが、これは一般的に手作業で解決するのは困難です。コンピューター代数システム\footnote{CAS, \emph{computer algebra system}}を利用して解析的な解を求めることはできますが、大きな利点はありません。したがって、数値的な方法で近似値を求める方が効率的です。
	
	像の位置を求めるための Python コード例は、
