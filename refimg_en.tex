...
Another approach is to determine the intersection point of the ray with the water surface using Fermat's principle. The abscissa of this intersection point can then be used as a parameter in the equation for the image location.

Both methods involve solving a quartic equation, which is generally difficult to solve analytically. While analytical solutions can be obtained using a computer algebra system (CAS), this approach does not offer a significant advantage. Finding approximate values using numerical methods is generally more efficient.
